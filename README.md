...
A comprehensive web-based image steganography application built with Streamlit that supports multiple steganography methods (LSB, Hybrid DCT, Hybrid DWT) with encryption capabilities, and batch processing.
...
- ✅ **Encryption**: AES-256-CBC message protection with PBKDF2-HMAC-SHA256 key derivation
- ✅ **Method Comparison**: Side-by-side comparison of all three steganography methods
- ✅ **Batch Processing**: 
  - Encode multiple images simultaneously (Basic & Advanced modes)
  - Decode ZIP archives with automatic method detection
  - Download results as ZIP, CSV, or individual images
  - Smart encryption detection (only decrypts when needed)
  - Support for encrypted and non-encrypted messages
- ✅ **Pixel Analysis**: Analyze optimal pixel distribution for message hiding
- ✅ **Watermarking**: Add visible text watermarks to images
- ✅ **Steganalysis**: Detect presence of hidden messages using ML
...
**Latest Changes**: Removed analytics module for cleaner codebase (stego, db, encryption, ui, batch_processing)
