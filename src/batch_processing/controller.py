...
class BatchProcessingController:
    ...
    def process_zip_batch(
        ...
        try:
            ...
            # Validate zip_path parameter
            if not isinstance(zip_path, (str, Path)):
                logger.error(f"Invalid zip_path type: {type(zip_path).__name__}")
                return {
                    'success': False,
                    'message': f"ZIP path must be string or Path, got {type(zip_path).__name__}"
                }
            ...
            if not zip_path.is_file():
                logger.error(f"ZIP path is not a file: {zip_path}")
                return {
                    'success': False,
                    'message': f"ZIP path is not a file: {zip_path}"
                }
            ...
            cleanup_ok = cleanup_extracted(str(self.input_images_path))
            if not cleanup_ok:
                logger.warning(f"Cleanup may have failed or found no files to clean for batch {batch_id}")
            ...
            logger.error(f"File/IO error during batch processing: {str(e)}", exc_info=True)
            return {
                'success': False,
                'message': f"File error during batch processing: {str(e)}"
            }
        except Exception as e:
    ...
    def process_image_batch(
        ...
        """
        try:
            # Validate image_paths parameter
            if not isinstance(image_paths, (list, tuple)):
                logger.error(f"Invalid image_paths type: {type(image_paths).__name__}")
                return {
                    'success': False,
                    'message': f"image_paths must be list or tuple, got {type(image_paths).__name__}"
                }
            
            if not image_paths:
                logger.error("image_paths cannot be empty")
                return {
                    'success': False,
                    'message': "image_paths cannot be empty"
                }
