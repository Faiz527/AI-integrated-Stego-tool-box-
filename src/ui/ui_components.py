...
def _is_likely_encrypted(message):
    """
    Check if an extracted message looks encrypted (base64-like binary).
    
    Encrypted messages from encrypt_message() are base64 encoded,
    so they only contain [A-Za-z0-9+/=] and are fairly random.
    
    Returns True if message looks encrypted, False if looks like plaintext.
    """
    ...
    try:
        ...
        printable_ratio = readable / len(text) if text else 0
        
        # If highly printable (>80%), it's likely plaintext, not encrypted
        if printable_ratio > 0.8:
            # Also check for non-base64 characters (which means it's plaintext)
            base64_chars = set('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=\n')
            non_b64 = sum(1 for c in text if c not in base64_chars)
            # If has non-base64 chars or spaces, it's definitely plaintext
            if non_b64 > 0 or ' ' in text:
                return False
        
        # If we got here, it might be encrypted base64
        return True
...
def _safe_display_message(message, max_chars=50):
    """
    Safely display a message, handling potential encoding issues.
    
    Args:
        message: str - the message to display
        max_chars: int - max characters to show (truncate with ...)
    
    Returns:
        str - safely displayable message (handles binary/encoding errors)
    """
    if not message:
    ...
    try:
        ...
        if isinstance(message, bytes):
            ...
            # If already string, try to encode and decode to verify
            message.encode('utf-8')
        
        # If we got here, it's valid UTF-8
        # Truncate to max_chars
        display_msg = message[:max_chars]
        ...
        # Message contains non-UTF8 data - show as binary indicator
        logger.debug(f"Message contains non-UTF8 data: {len(message)} bytes")
        try:
            # Try to show hex representation
            if isinstance(message, str):
                message_bytes = message.encode('latin1', errors='ignore')
            else:
                message_bytes = message
            
            hex_preview = message_bytes[:20].hex().upper()
            return f"[Binary: {hex_preview}...]"
        except:
            return "[Binary data]"
    except Exception as e:
        logger.warning(f"Error displaying message: {str(e)}")
...
def _show_batch_encode_section():
    ...
    with tab_setup:
        ...
        # Process button
        if st.button("▶️ Start Batch Encoding", width='stretch', type="primary"):
            if not uploaded_files or not message:
                ...
                _perform_basic_batch_encode(uploaded_files, upload_type, message, method, 
    ...
    with tab_results:
        if "batch_encode_results" in st.session_state:
            ...
            # Add download options for encoded images
            if "batch_encoded_images" in st.session_state and st.session_state.batch_encoded_images:
                st.divider()
                ...
                with col1:
                    zip_buffer = _create_zip_from_images(st.session_state.batch_encoded_images)
                    st.download_button(
                ...
                with col2:
                    if st.button("🖼️ Preview Images", width='stretch', key="preview_btn"):
                ...
                if st.session_state.get("show_encoded_preview", False):
                    st.divider()
                    st.markdown("### Preview of Encoded Images")
                    preview_cols = st.columns(3)
                    for idx, (filename, image_data) in enumerate(st.session_state.batch_encoded_images.items()):
                        with preview_cols[idx % 3]:
                            st.image(image_data, caption=filename, width='stretch')
                            with st.expander(f"Download {filename}"):
                                img_buffer = BytesIO()
...
def _show_batch_decode_section():
    ...
    with tab_results:
        if "batch_decode_results" in st.session_state:
            ...
            results_df = pd.DataFrame(st.session_state.batch_decode_results)
            # Display only preview columns in the dataframe
            display_cols = ["filename", "status", "method", "message"]
            st.dataframe(results_df[display_cols], width='stretch')
            
            # Display full messages in expandable sections
            for result in st.session_state.batch_decode_results:
                if result["status"] == "✅ Found":
                    full_msg = result.get("full_message", result.get("message", ""))
                    if full_msg:
...
def _create_zip_from_images(images_dict):
    """Create a ZIP file from a dictionary of PIL Image objects."""
    zip_buffer = BytesIO()
...
def _perform_basic_batch_encode(uploaded_files, upload_type, message, method, use_encryption, encryption_password):
    """Perform basic batch encoding (same message in all images)."""
    try:
        # Handle ZIP file extraction
        if upload_type and "ZIP" in upload_type:
            try:
                ...
                for file_path in Path(extract_dir).rglob('*'):
                    if file_path.is_file() and file_path.suffix.lower() in ['.png', '.jpg', '.jpeg', '.bmp', '.gif']:
                        image_files.append(str(file_path))
                
                if not image_files:
                    show_error("No image files found in ZIP")
                    return
                
                is_from_zip = True
            except zipfile.BadZipFile:
                show_error("Invalid ZIP file format")
                return
            except Exception as e:
                logger.error(f"ZIP extraction error: {str(e)}", exc_info=True)
                show_error(f"Failed to extract ZIP: {str(e)}")
                return
        else:
            # Normalize uploaded_files: handle single UploadedFile or list
        ...
        for i, image_file in enumerate(image_files):
            status_text = f"Processing {i+1}/{len(image_files)}..."
            ...
            try:
                ...
                # Store encoded image for download
                encoded_images_dict[f"encoded_{filename}"] = encoded_image
                ...
                display_filename = filename if is_from_zip else image_file.name
                logger.error(f"File error for {display_filename}: {str(e)}")
                results.append({
                    "filename": display_filename,
                    "status": f"❌ File error: {str(e)[:30]}",
                    "method": method
                })
            except ValueError as e:
                display_filename = filename if is_from_zip else image_file.name
                logger.error(f"Encoding error for {display_filename}: {str(e)}")
                results.append({
                    "filename": display_filename,
                    "status": f"❌ Encoding error: {str(e)[:30]}",
                    "method": method
                })
            except Exception as e:
                display_filename = filename if is_from_zip else image_file.name
                logger.error(f"Unexpected error for {display_filename}: {str(e)}", exc_info=True)
                results.append({
                    "filename": display_filename,
        ...
        st.session_state.batch_encoded_images = encoded_images_dict
        show_success(f"Batch encoding complete! {len([r for r in results if '✅' in r['status']])}/{len(image_files)} successful")
...
def _perform_advanced_batch_encode(uploaded_files, upload_type, message, method, 
    ...
    try:
        ...
        # Recalculate file_count from actual uploaded files (fixes data loss from zip() truncation)
        actual_file_count = len(image_files)
        if actual_file_count < 2:
            show_error("Advanced Mode requires at least 2 images")
            return
        ...
        # Split message into chunks based on actual file count
        chunk_size = len(message_to_split) // actual_file_count
        message_chunks = [
            message_to_split[i*chunk_size:(i+1)*chunk_size] 
            for i in range(actual_file_count)
        ]
        
        for i, (image_file, chunk) in enumerate(zip(image_files, message_chunks)):
            status_text = f"Processing {i+1}/{actual_file_count}..."
            st.write(status_text)
            ...
            progress.progress((i + 1) / actual_file_count)
        
        st.session_state.batch_encode_results = results
        st.session_state.batch_encoded_images = encoded_images_dict
        show_success(f"Advanced batch encoding complete! {len([r for r in results if '✅' in r['status']])}/{actual_file_count} successful")
        
    except Exception as e:
        logger.error(f"Advanced batch encoding failed: {str(e)}", exc_info=True)
...
def _perform_batch_decode(uploaded_files, upload_type, use_encryption, decryption_password):
    ...
    try:
        ...
        if "ZIP" in upload_type and len(uploaded_files) > 0:
            is_from_zip = True
            try:
                logger.info(f"Attempting to extract ZIP file: {uploaded_files[0].name}")
                zip_buffer = BytesIO(uploaded_files[0].read())
                zip_buffer.seek(0)
                
                with zipfile.ZipFile(zip_buffer, 'r') as zip_ref:
                    logger.info(f"ZIP opened successfully. Found {len(zip_ref.filelist)} files")
                    for file_info in zip_ref.filelist:
                        if file_info.filename.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.gif')):
                            try:
                                file_data = zip_ref.read(file_info.filename)
                                logger.debug(f"Read {len(file_data)} bytes from {file_info.filename}")
                                img = Image.open(BytesIO(file_data))
                                img.load()  # Force load to verify image is valid
                                filename = os.path.basename(file_info.filename)
                                extracted_images.append((img, filename))
                                logger.info(f"Successfully extracted image: {filename}")
                            except Exception as e:
                                logger.warning(f"Could not open {file_info.filename}: {str(e)}")
                
                if extracted_images:
                    logger.info(f"Successfully extracted {len(extracted_images)} images from ZIP")
                    uploaded_files = extracted_images
                else:
                    logger.error("No valid images found in ZIP file")
                    show_error("No valid images found in ZIP file")
                    return
            except zipfile.BadZipFile as e:
                logger.error(f"ZIP file is corrupted: {str(e)}", exc_info=True)
                show_error(f"ZIP file is corrupted: {str(e)}")
                return
            except Exception as e:
                logger.error(f"ZIP extraction failed: {str(e)}", exc_info=True)
                show_error(f"Failed to extract ZIP file: {str(e)}")
                return
        ...
        for i, image_file in enumerate(uploaded_files):
            ...
            try:
                # Handle both ZIP extraction (tuple with PIL Image and filename) and direct upload (UploadedFile)
                if is_from_zip:
                ...
                for method_name, method_func in methods:
                    try:
                        ...
                        logger.debug(f"Method {method_name} failed for {filename}: {str(e)}")
                        continue
                    except Exception as e:
                        logger.debug(f"Unexpected error with {method_name} for {filename}: {str(e)}")
                        continue
                
                if decoded_message and method_used:
                    # Only attempt decryption if:
                    # 1. User checked the encryption checkbox, AND
                    # 2. Provided a password, AND  
                    # 3. The extracted message LOOKS encrypted
                    if use_encryption and decryption_password and _is_likely_encrypted(decoded_message):
                        try:
                            decrypted = decrypt_message(decoded_message, decryption_password)
                            decoded_message = decrypted
                            logger.info(f"Successfully decrypted message from {filename}")
                        except (ValueError, TypeError) as e:
                            logger.warning(f"Decryption failed for {filename}: {str(e)}. Message doesn't appear to be encrypted, using raw extracted message.")
                            # Keep the raw extracted message - it's likely not encrypted
                        except Exception as e:
                            logger.error(f"Unexpected error during decryption for {filename}: {str(e)}")
                            # Keep the raw extracted message
                    elif use_encryption and decryption_password and not _is_likely_encrypted(decoded_message):
                        logger.info(f"Message from {filename} doesn't look encrypted (looks like plaintext). Skipping decryption.")
                    
                    results.append({
                        "filename": filename,
                        "status": "✅ Found",
                        "method": method_used,
                        "message": _safe_display_message(decoded_message, max_chars=50),
                        "full_message": decoded_message  # Store full message separately
                    })
                else:
                    results.append({
                        "filename": filename,
                ...
                logger.error(f"File error for {filename}: {str(e)}")
                results.append({
                    "filename": filename,
                    "status": f"❌ File error",
                    "method": "N/A",
        ...
        logger.error(f"Batch decoding failed: {str(e)}", exc_info=True)
